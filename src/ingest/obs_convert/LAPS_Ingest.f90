...
!==============================================================================
  ...
  INTEGER :: MAXSTN,STATUS		! MAXIMUM STATIONS
  ...
  LOGICAL,     ALLOCATABLE, DIMENSION(:) :: MASK_WIND, MASK_TMP, MASK_DEW
  ...
  ALLOCATE(OBTIME(MAXSTN),WMOIDS(MAXSTN),STNAME(MAXSTN), &
           ...
           OBSWND(2,MAXSTN),ERRWND(2,MAXSTN), MASK_WIND(MAXSTN),MASK_TMP(MAXSTN),MASK_DEW(MAXSTN), STAT=STATUS)
  IF (STATUS .NE. 0) THEN
  ...
  ! CONVERT WIND DIRECTION AND SPEED INTO U AND V USING ARRAY OPERATIONS:
  IF (NOBBOX > 0) THEN
    MASK_WIND(1:NOBBOX) = (OBSDIR(1:NOBBOX) .NE. RVALUE_MISSING) .AND. &
                          (OBSDIR(1:NOBBOX) .NE. SFCOBS_INVALID) .AND. &
                          (OBSSPD(1:NOBBOX) .NE. RVALUE_MISSING) .AND. &
                          (OBSSPD(1:NOBBOX) .NE. SFCOBS_INVALID)

    OBSWND(:,1:NOBBOX) = RVALUE_MISSING
    ERRWND(:,1:NOBBOX) = RVALUE_MISSING

    WHERE (MASK_WIND(1:NOBBOX))
      OBSWND(1,1:NOBBOX) = -SIND(OBSDIR(1:NOBBOX)) * OBSSPD(1:NOBBOX) * 0.514791
      OBSWND(2,1:NOBBOX) = -COSD(OBSDIR(1:NOBBOX)) * OBSSPD(1:NOBBOX) * 0.514791
      ERRWND(1,1:NOBBOX) = -SIND(ERRDIR(1:NOBBOX)) * ERRSPD(1:NOBBOX) * 0.514791
      ERRWND(2,1:NOBBOX) = -COSD(ERRDIR(1:NOBBOX)) * ERRSPD(1:NOBBOX) * 0.514791
    END WHERE

    MASK_TMP(1:NOBBOX) = (OBSTMP(1:NOBBOX) .NE. RVALUE_MISSING) .AND. &
                         (OBSTMP(1:NOBBOX) .NE. SFCOBS_INVALID)
    WHERE (MASK_TMP(1:NOBBOX))
      OBSTMP(1:NOBBOX) = (OBSTMP(1:NOBBOX) - 32.0) * 5.0 / 9.0
    END WHERE

    MASK_DEW(1:NOBBOX) = (OBSDEW(1:NOBBOX) .NE. RVALUE_MISSING) .AND. &
                         (OBSDEW(1:NOBBOX) .NE. SFCOBS_INVALID)
    WHERE (MASK_DEW(1:NOBBOX))
      OBSDEW(1:NOBBOX) = (OBSDEW(1:NOBBOX) - 32.0) * 5.0 / 9.0
    END WHERE
  END IF
  ...
  DEALLOCATE(OBTIME,WMOIDS,STNAME, &
             ...
             ERRPCP,ERRSNW, &
             CLDAMT,CLDHGT, &
             OBSWND,ERRWND,MASK_WIND,MASK_TMP,MASK_DEW, STAT=STATUS)
  IF (STATUS .NE. 0) THEN
