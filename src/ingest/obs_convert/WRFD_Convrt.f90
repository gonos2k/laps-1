...
2   FORMAT(A4,A2)
    ...
    IF (PSSFCOBS(I) .EQ. RVALUE_MISSING .OR. PSSFCOBS(I) .EQ. SFCOBS_INVALID) PSSFCOBS(I) = MISSNG
    IF (TMPSFCOB(I) .EQ. RVALUE_MISSING .OR. TMPSFCOB(I) .EQ. SFCOBS_INVALID) TMPSFCOB(I) = MISSNG
    IF (RHSFCOBS(I) .EQ. RVALUE_MISSING .OR. RHSFCOBS(I) .EQ. SFCOBS_INVALID) RHSFCOBS(I) = MISSNG
    WHERE (HEIGHTOB(I,1:NL) .EQ. RVALUE_MISSING) HEIGHTOB(I,1:NL) = MISSNG
    END WHERE
    WHERE (UWINDOBS(I,1:NL) .EQ. RVALUE_MISSING) UWINDOBS(I,1:NL) = MISSNG
    END WHERE
    WHERE (VWINDOBS(I,1:NL) .EQ. RVALUE_MISSING) VWINDOBS(I,1:NL) = MISSNG
    END WHERE
    DO J=1,NL
      IF (J .GT. 1) THEN
        PSSFCOBS(I) = MISSNG
